...
melissa_var_analysis <- function(opts, sim){
    # Initialize lists
    melissa_prof = melissa_mean = eval_perf <- vector("list", length = length(opts$cluster_var))
    i <- 1
    # Iterate
...
obj <- parallel::mclapply(X = 1:opts$total_sims, FUN = function(sim)
    melissa_var_analysis(opts = opts, sim = sim), mc.cores = no_cores_out)
print(date())
