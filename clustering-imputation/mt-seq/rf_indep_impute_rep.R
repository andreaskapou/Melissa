...
##------------------------------------
io <- list(dataset = "mt-seq", data_file = "prom3k", cov = 10, sd = 0.2)
io$data_dir = "../../local-data/melissa/"
io$out_dir = paste0(io$data_dir, io$dataset, "/imputation/")
dt <- readRDS(paste0(io$data_dir, "filtered_met/", io$dataset, "/", io$data_file, "_cov",
...
saveRDS(obj, file = paste0(io$out_dir, "rf_indep_sim", opts$total_sims,
                           "_", io$data_file,
                           "_cov", io$cov,
                           "_sd", io$sd,
                           "_M", opts$M,
