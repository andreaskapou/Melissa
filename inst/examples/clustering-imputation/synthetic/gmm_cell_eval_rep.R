...
gmm_var_analysis <- function(opts, sim){
  # Initialize lists
  gmm = eval_perf <- vector("list", length = length(opts$N))
  # Load synthetic data
  ...
  for (cl_iter in opts$N) {
    ...
    X <- matrix(0, nrow = cl_iter, ncol = opts$M)
    ...
    act_obs   <- vector(mode = "list", length = cl_iter) # Keep actual CpG states
    pred_obs  <- vector(mode = "list", length = cl_iter) # Keep predicted CpG states
    for (n in 1:cl_iter) {
      # Cluster assignment
