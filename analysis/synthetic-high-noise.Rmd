---
title: "Melissa - Simulation study synthetic high noise data"
author:
...
# Load synthetic data
cpg_data <- readRDS(paste0(io$cpg_dir, "raw/high_noise_encode_data.rds")) 
# Load joint analysis results
dt_melissa_cpg_prcg <- readRDS(paste0(io$cpg_dir, "high_noise_encode_melissa_K", io$K, "_rbf", io$basis, 
                                "_dataTrain", io$data_prcg, 
                                "_regionTrain", io$reg_prcg, 
                                "_clusterVar", io$cl_var, ".rds"))

# Load independent analysis results
indep_cpg_prcg <- readRDS(paste0(io$cpg_dir, "high_noise_encode_indep_K", io$K, "_rbf", io$basis,
                                "_dataTrain", io$data_prcg,
                                "_regionTrain", io$reg_prcg,
                                "_clusterVar", io$cl_var, ".rds"))

# Load RF analysis results
rf_cpg_prcg <- readRDS(paste0(io$cpg_dir, "high_noise_encode_rf_indep_K", io$K, "_rbf", io$basis,
                                "_dataTrain", io$data_prcg,
                                "_regionTrain", io$reg_prcg,
                                "_clusterVar", io$cl_var, ".rds"))
...
for (i in 1:length(dt_melissa_cpg_prcg)) {     # Itrate over simulations
  for (m in 1:length(io$cpg_analysis)) {  # Iterate over CpG percentages
    ...
    dt <- data.table(cpg_cov = io$cpg_analysis[m],
                     ...
                     melissa_rate_ari = cluster_ari(cpg_data$synth_data[[i]]$C_true, dt_melissa_cpg_prcg[[i]]$encode[[m]]$r_nk),
                     melissa_error = cluster_error(cpg_data$synth_data[[i]]$C_true, dt_melissa_cpg_prcg[[i]]$melissa_prof[[m]]$r_nk),
                     melissa_rate_error = cluster_error(cpg_data$synth_data[[i]]$C_true, dt_melissa_cpg_prcg[[i]]$encode[[m]]$r_nk)
                     )
...
auc_cpg_jitter <- auc_cpg_jitter %>% .[, c("cpg_cov", "auc_melissa", "auc_melissa_rate", "auc_indep_prof", "auc_indep_rate", "auc_rf") ] %>% setnames(c("cpg_cov", "auc_melissa", "auc_melissa_rate", "auc_indep_prof", "auc_indep_rate", "auc_rf"), c("x", "Melissa", "Melissa Rate", "BPRMeth", "Rate", "RF")) %>% .[, x := as.factor(x)] %>% melt(variable.name = "Model", value.name = "y") %>% .[, Model := factor(Model, levels = c("Melissa", "BPRMeth", "RF", "Melissa Rate", "Rate"))]
...
f_cpg_jitter <- f_cpg_jitter %>% .[, c("cpg_cov", "f_melissa", "f_melissa_rate", "f_indep_prof", "f_indep_rate", "f_rf") ] %>% setnames(c("cpg_cov", "f_melissa", "f_melissa_rate", "f_indep_prof", "f_indep_rate", "f_rf"), c("x", "Melissa", "Melissa Rate", "BPRMeth", "Rate", "RF")) %>% .[, x := as.factor(x)] %>% melt(variable.name = "Model", value.name = "y") %>% .[, Model := factor(Model, levels = c("Melissa", "BPRMeth", "RF", "Melissa Rate", "Rate"))]
...
# Load joint analysis results
dt_melissa_cl_var <- readRDS(paste0(io$var_dir, "high_noise_encode_melissa_K", io$K, "_rbf", io$basis, 
                                "_dataTrain", io$data_prcg, 
                                "_regionTrain", io$reg_prcg, 
                                "_cpgTrain", io$cpg_prcg, ".rds"))
# Load independent analysis results
indep_cl_var <- readRDS(paste0(io$var_dir, "high_noise_encode_indep_K", io$K, "_rbf", io$basis, 
                                "_dataTrain", io$data_prcg, 
                                "_regionTrain", io$reg_prcg, 
                                "_cpgTrain", io$cpg_prcg, ".rds"))
# Load RF analysis results
rf_cl_var <- readRDS(paste0(io$var_dir, "high_noise_encode_rf_indep_K", io$K, "_rbf", io$basis, 
                                "_dataTrain", io$data_prcg, 
                                "_regionTrain", io$reg_prcg, 
                                "_cpgTrain", io$cpg_prcg, ".rds"))
...
for (i in 1:length(dt_melissa_cl_var)) {
  for (m in 1:length(io$cluster_var_analysis)) {
    # Load synthetic data
    var_data <- readRDS(paste0(io$var_dir, "raw/data-sims/high_noise_encode_data_", io$cluster_var_analysis[m], "_", i, ".rds")) 
...
auc_var_jitter <- auc_var_jitter %>% .[, c("cell_var", "auc_melissa", "auc_melissa_rate", "auc_indep_prof", "auc_indep_rate", "auc_rf") ] %>% setnames(c("cell_var", "auc_melissa", "auc_melissa_rate", "auc_indep_prof", "auc_indep_rate", "auc_rf"), c("x", "Melissa", "Melissa Rate", "BPRMeth", "Rate", "RF")) %>% .[, x := as.factor(x)] %>% melt(variable.name = "Model", value.name = "y") %>% .[, Model := factor(Model, levels = c("Melissa", "BPRMeth", "RF", "Melissa Rate", "Rate"))]
...
pdf(file = paste0("out/synthetic/high-noise-auc-synth.pdf"), width = 17, height = 6, useDingbats = FALSE)
final_fig_auc
dev.off()
```
...
pdf(file = paste0("out/synthetic/high-noise-f-synth.pdf"), width = 17, height = 6, useDingbats = FALSE)
final_fig_f
dev.off()
```
...
pdf(file = paste0("out/synthetic/high-noise-perf-synth.pdf"), width = 14, height = 10, useDingbats = FALSE)
