...
for (cpg_prcg in io$cpg_prcg) {
   ...
   dt_melissa <- readRDS(paste0(io$data_dir, "melissa_sim10_", io$regions, 
                              ...
                              "_basis", 13, "_dataPrcg", io$data_prcg, 
                              "_regionPrcg", io$reg_prcg, "_cpgPrcg", 0.5,
   ...
   for (i in 1:length(dt_melissa$model)) {
       ...
       dt <- data.table(region = io$regions, cpg_prcg = cpg_prcg,
                     auc_melissa = auc_melissa,
                     auc_melissa_rate = auc_melissa_rate,
...
dt_boxplot <- dt_boxplot[, c("auc_melissa", "auc_melissa_rate", "auc_indep_prof", "auc_indep_rate", "auc_rf", "auc_deepcpg", "auc_deepcpg_sub") := 
                             list(auc_melissa + rnorm(.N, 0, s2), auc_melissa_rate + rnorm(.N, 0, s2),
                                  ...
                                  auc_rf + rnorm(.N, 0, s2), auc_deepcpg + rnorm(.N, 0, s2/4))]
...
dt_boxplot <- dt_boxplot %>% setnames(c("cpg_prcg", "auc_melissa", "auc_melissa_rate", "auc_indep_prof", "auc_indep_rate", "auc_rf", "auc_deepcpg"), c("x", "Melissa", "Melissa Rate", "Indep Profile", "Indep Rate", "RF", "DeepCpG")) %>% .[, x := as.factor(x)] %>% melt(variable.name = "Model", value.name = "y")  %>% .[, x := factor(x, levels = c("20%", "50%", "80%"))] %>% .[, Model := factor(Model, levels = c("Melissa", "DeepCpG", "Indep Profile", "RF", "Melissa Rate", "Indep Rate"))]
...
pdf(file = paste0("out/", io$dataset, io$sub_dir, "/auc-encode.pdf"), width = 15, height = 8, useDingbats = FALSE)
p_auc_box
dev.off()
...
dt_boxplot <- dt_boxplot[, c("f_melissa", "f_melissa_rate", "f_indep_prof", "f_indep_rate", "f_rf", "f_deepcpg") := 
                             list(f_melissa + rnorm(.N, 0, s2), f_melissa_rate + rnorm(.N, 0, s2),
                                  ...
                                  f_rf + rnorm(.N, 0, s2), f_deepcpg + rnorm(.N, 0, s2))]
...
p_f_box <- ggplot(dt_boxplot, aes(x = x, y = y, fill = Model)) +
  ...
  scale_fill_manual(values = c("red3", "darkgreen", "chocolate2", "dodgerblue4", "mediumorchid4", "mistyrose4")) + 
  #scale_x_discrete(labels = c("prom3k" = "Promoter 3kb", "prom5k" = "Promoter 5kb", "prom10k" = "Promoter 10kb", "active_enhancers" = "Active enhancers", "Nanog" = "Nanog", "super_enhancers" = "Super enhancers")) +
  scale_y_continuous(limits = c(0.44, 0.75), breaks = pretty_breaks(n = 4)) +
  labs(title = "", x = "Coverage", y = "F-measure") +
  boxplot_theme()
print(p_f_box)
...
colnames(dt_boxplot) <- c("Region", "Melissa", "Melissa Rate", "Indep Profile", "Indep Rate", "RF", "DeepCpG", "DeepCpG Sub")
...
# Rename and refactor
dt_tpr_fpr <- dt_tpr_fpr %>% .[, c("Region", "Model") := list(as.factor(Region), as.factor(Model))] %>% .[, Region := factor(Region, levels = c("Promoter 10kb", "Promoter 5kb", "Promoter 3kb", "Nanog", "Super enhancers", "Active enhancers"))] %>% .[, Model := factor(Model, levels = c("Melissa", "Melissa Rate", "Indep Profile", "Indep Rate", "RF", "DeepCpG", "DeepCpG Sub"))] %>% 
    .[, Model := factor(Model, levels = c("Melissa", "DeepCpG", "DeepCpG Sub", "Indep Profile", "RF", "Melissa Rate", "Indep Rate"))]
...
# Rename and refactor
dt_pr <- dt_pr %>% .[, c("Region", "Model") := list(as.factor(Region), as.factor(Model))] %>% .[, Region := factor(Region, levels = c("Promoter 10kb", "Promoter 5kb", "Promoter 3kb", "Nanog", "Super enhancers", "Active enhancers"))] %>% .[, Model := factor(Model, levels = c("Melissa", "Melissa Rate", "Indep Profile", "Indep Rate", "RF", "DeepCpG", "DeepCpG Sub"))] %>% 
    .[, Model := factor(Model, levels = c("Melissa", "DeepCpG", "DeepCpG Sub", "Indep Profile", "RF", "Melissa Rate", "Indep Rate"))]
...
# Load Melissa model
melissa_obj <- readRDS(paste0(io$data_dir, "melissa_sim10_", context, 
...
for (t in 1:length(regions)) {
    ...
    region_melissa_obj[[t]] <- list(W = w_prof[r,,cluster_assinments], 
                                    W_Sigma = melissa$W_Sigma[[r]][cluster_assinments],
                                    basis = melissa$basis)
