...
# Rename and refactor
dt_tpr_fpr <- dt_tpr_fpr %>% .[, c("cpg_prcg", "Model") := list(as.factor(cpg_prcg), as.factor(Model))] %>% .[, Region := factor(cpg_prcg, levels = c("20%", "50%", "80%"))] %>% .[, Model := factor(Model, levels = c("Melissa", "Melissa Rate", "Indep Profile", "Indep Rate", "RF", "DeepCpG"))] %>% 
...
# Rename and refactor
dt_pr <- dt_pr %>% .[, c("cpg_prcg", "Model") := list(as.factor(cpg_prcg), as.factor(Model))] %>% .[, cpg_prcg := factor(cpg_prcg, levels = c("20%", "50%", "80%"))] %>% .[, Model := factor(Model, levels = c("Melissa", "Melissa Rate", "Indep Profile", "Indep Rate", "RF", "DeepCpG"))] %>% 
    .[, Model := factor(Model, levels = c("Melissa", "DeepCpG", "Indep Profile", "RF", "Melissa Rate", "Indep Rate"))]
...
pdf(file = paste0("out/ENCODE/auc-f-encode.pdf"), width = 17, height = 6, useDingbats = FALSE)
final_fig_f
dev.off()
```
...
# Final plot
```{r joint_plot, fig.wide=TRUE, eval=FALSE, echo=FALSE, include=TRUE, warning=FALSE, message=FALSE, fig.height=10, fig.width=14.5}
