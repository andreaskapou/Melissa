...
sub_dir   <- "subsampled/"
#sub_dir   <- "/"
data_dir  <- paste0("../../local-data/", dataset, "met/deepcpg/processed/unfiltered/", sub_dir)
...
# Update options
opts <- obj$opts
opts$filter_chr <- c("3", "12", "14", "17", "19")
opts$cov <- 15          # CpG density at each source
#opts$cov_deepcpg <- 5   # CpG density at each source
...
init_obj <- readRDS(paste0(out_dir, data_file, "_cov", opts$cov, "_sd", opts$met_sd, "_gene_var5.rds"))
...
obj <- list(met = met, anno_region = anno_region, annos = annos, rna = init_obj$rna, opts = opts, io = obj$io)
saveRDS(obj, file = paste0(out_dir, "deepcpg/", sub_dir, data_file, "_cov", opts$cov, "_sd",
                           opts$met_sd, "_gene_var5.rds"))
